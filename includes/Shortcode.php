...
class Shortcode {
	...
	private function __construct() {
		...
		add_shortcode( 'wpr-future-events', array( $this, 'shortcode' ) );
	}
	...
	public function shortcode( $atts = []) {
		...
		$data1 = get_option('wpr_future_events');
		
		
		$data = array_merge($data1, $atts); 
		...
		$object = shortcode_atts( array(
			...
			'future_data'    => $data,
		), $atts, 'wpr-future-events' );
		

		wp_localize_script( $this->plugin_slug . '-shortcode-script', $object_name, $object );

		$shortcode = '<div class="wpr-future-shortcode" data-object-id="' . $object_name . '"></div>';
		return $shortcode;
	}
}
